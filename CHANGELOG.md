...
- **BearClaw contract moved out of `AGENTS.md`** — `AGENTS.md` is now a compact repo guide, while the BearClaw-specific edge contract, auth/header rules, storage notes, rate limiting guidance, and sticky upstream behavior now live in `examples/bearclaw/README.md` alongside the canonical example config and env bundle.
- **`CONTRIBUTING.md` reduced to public contributor guidance** — internal workflow references such as `BLINK.md` and `ROADMAP.md` were removed, leaving only generic expectations and the standard test commands.
