...
- **Production dependency boundary enforcement (#651)** — production source,
  ...
  native-only implementation boundary rather than the old TLS-library deny
  list. The source audit has fixtures for forbidden production FFI, system
  linkage, vendored C/C++, runtime loading, package metadata, multiline
  Docker installs, indirect build linkage, and nominally test-named helper
  bypasses; the binary audit rejects dynamic dependencies outside the narrow
  OS/runtime substrate allowlist.
...
- **Response compression now active for static file serving (#142)** — `compressResponse` (gzip via `std.compress.flate`; Brotli reserved until a native implementation exists) was implemented with full config wiring but never called when serving static responses. The buffered static path (TLS connections and any non-sendfile response) now compresses the response body when `TARDIGRADE_COMPRESSION_ENABLED=true` and the client advertises `Accept-Encoding: gzip`. Adds `Content-Encoding` and `Vary: Accept-Encoding` headers on compressed responses. File-backed sendfile responses are intentionally left uncompressed (they target the zero-copy plain-HTTP path). HTTP/3 static responses gain the same gzip compression.
