...
### Fixed
- **Canonical perf target route set drifted from the benchmark harness (#100)** — the `tardigrade-perf` guest only exposed `/proxy/payload-64k.bin` by default even though the harness and upstream fixture had already grown a `256 KiB` scenario. The canonical release-baseline metadata and docs now match the live guest route set, `release-baseline.sh` includes both payload-size scenarios by default, and the upstream fixture remains the required `tardigrade-upstream.service` backend for canonical runs. Verified live on May 13, 2026 inside `tardigrade-perf`: `GET /proxy/health` returned `200` with `Content-Length: 2`, `GET /proxy/payload-64k.bin` returned `200` with `Content-Length: 65536`, and `GET /proxy/payload-256k.bin` returned `200` with `Content-Length: 262144`. Closes #100.
...
- Proxy payload-scaling benchmark report and artifacts for the canonical homelab loopback target — added `benchmarks/results/2026-05-13/payload-scaling-loopback.md`, the saved JSON result set, and a host-side syscall hotspot sample for the `256 KiB` proxy case to guide the next round of payload-path optimization work.
