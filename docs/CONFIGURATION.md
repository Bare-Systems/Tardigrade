...
| `TARDIGRADE_TRUSTED_PROXY_CIDRS` | CSV CIDRs | `[]` | Trusted proxy ranges (e.g. a Docker bridge gateway for a host-local cloudflared). A peer inside a range may supply forwarded-client metadata, and `X-Forwarded-For` entries inside a range are skipped when resolving the client address from the right. | `TARDIGRADE_TRUSTED_PROXY_CIDRS=172.18.0.0/16,10.0.0.0/8` |
| `TARDIGRADE_REAL_IP_HEADER` | header name | `""` | Header a trusted CDN/proxy sets to the client address (e.g. `CF-Connecting-IP`, `True-Client-IP`); used only from a trusted peer and only when it holds an IP literal. Takes precedence over `X-Forwarded-For`. | `TARDIGRADE_REAL_IP_HEADER=CF-Connecting-IP` |
