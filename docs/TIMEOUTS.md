...
| Upstream TLS handshake | (bounded by connect/response knobs) | — | `SO_RCVTIMEO`/`SO_SNDTIMEO` set before `UpstreamTlsConn.connect`: h1 paths since #196/#232, **h2/h2c pool since #171** | handshake error → 502 | ✅ *(the h2 pool previously handshook with no deadline — a TCP-accepting-but-silent origin hung the worker)* |
