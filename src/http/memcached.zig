...
pub fn parseEndpoint(raw: []const u8) !Endpoint {
    ...
    const colon = std.mem.findScalarLast(u8, trimmed, ':') orelse return error.InvalidEndpoint;
...
pub fn get(allocator: std.mem.Allocator, endpoint: []const u8, key: []const u8) !?[]u8 {
    ...
    const value_start = std.mem.find(u8, resp, "\r\n") orelse return error.InvalidResponse;
    const value_end = std.mem.findPos(u8, resp, value_start + 2, "\r\nEND\r\n") orelse return error.InvalidResponse;
...
pub fn set(allocator: std.mem.Allocator, endpoint: []const u8, key: []const u8, value: []const u8, ttl: u32) !bool {
    ...
    const n = try stream.read(&buf);
    if (n == 0) return false;
    return std.mem.find(u8, buf[0..n], "STORED") != null;
}
