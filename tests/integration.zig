...
const TardigradeProcess = struct {
    ...
    fn spawn(allocator: std.mem.Allocator, options: TardigradeOptions) !TardigradeProcess {
        ...
        if (env_map.get("TARDIGRADE_TLS_CERT_PATH") != null and
...
fn prepareBearClawFixture(
    ...
    const fixture_name = "native_ed25519";
    ...
    while (true) {
        ...
        if (header_end == null) {
            if (std.mem.find(u8, raw_buf.items, "\r\n\r\n")) |idx| {
                ...
                } else if (headerValue(headers_raw, "Transfer-Encoding")) |te| {
                    if (std.ascii.eqlIgnoreCase(std.mem.trim(u8, te, " \t\r\n"), "chunked")) {
                        target_len = chunkedEncodedMessageEnd(raw_buf.items[idx + 4 ..]) catch |err| switch (err) {
                            error.IncompleteChunkedBody => null,
                            else => return err,
                        };
                        if (target_len) |body_end| target_len = idx + 4 + body_end;
                    }
                }
            }
        } else if (target_len == null) {
            const idx = header_end.?;
            const headers_raw = raw_buf.items[0 .. idx + 2];
...
fn chunkedEncodedMessageEnd(data: []const u8) !?usize {
    var pos: usize = 0;
    while (true) {
        const line_end_rel = std.mem.find(u8, data[pos..], "\r\n") orelse return error.IncompleteChunkedBody;
        ...
        if (chunk_len == 0) {
            while (true) {
                const trailer_end_rel = std.mem.find(u8, data[pos..], "\r\n") orelse return error.IncompleteChunkedBody;
                if (trailer_end_rel == 0) return pos + 2;
                pos += trailer_end_rel + 2;
            }
        }
        if (pos + chunk_len + 2 > data.len) return error.IncompleteChunkedBody;
        pos += chunk_len;
...
fn readHttpHeadersOnly(allocator: std.mem.Allocator, stream: compat.NetStream) ![]u8 {
    ...
    while (true) {
        const n = try readSocketWithPoll(stream.handle, &tmp, 5_000);
...
fn waitUntilReady(port: u16, log_path: []const u8, options: TardigradeOptions) !void {
    ...
    while (attempts < 100) : (attempts += 1) {
        if (ready_over_https) {
            var resp = (if (options.ready_client_cert == null and options.ready_client_key == null)
...
/// #649: every shipping profile is native TLS now, so this always passes.
/// Kept (rather than deleted at each of its ~60 call sites) as the
/// existing marker of "this case assumes/exercises the native TLS engine".
...
/// hold only on `-Dtls-profile=appliance`. The general-purpose profile
/// shares the same native TLS engine but uses the permissive generic
/// credential store, so these behaviors are intentionally absent.
fn requireApplianceTlsProfile() !void {
    if (!std.mem.eql(u8, build_options.tls_profile, "appliance")) return error.SkipZigTest;
}

/// #634/#649: the general-purpose profile (`-Dtls-profile=general`, the
/// default) only — for cases proving generic-native behavior whose
/// appliance counterpart takes a different code path with its own
/// assertions (e.g. the appliance credential-config reload rejection vs.
/// the generic native TLS-topology reload rejection).
...
/// in `edge_gateway.zig` whenever TLS files and HTTP/3 are configured, in
/// every profile since #649) is a multi-identity SNI provider with
...
// into a failing SIGHUP: every TLS profile is native since #649, and
// inspecting `edge_gateway.run`/`gateway_shutdown.hotReloadConfig` shows the
// appliance profile's real served identity
// (`appliance_credentials.ApplianceCredentials`, held in
// `appliance_identity`) is loaded once at startup and never touched by
// `hotReloadConfig` at all -- the `native_credentials`/`NativeCredentialStore`
// two-phase prepare/commit path this criterion originally had in mind is
// wired up only for the non-appliance (general) profile, so it is never
// reachable here regardless of what this test attempts.
// `rotation.persistent.quic_credential_reload_atomicity` below exercises
// that real path instead, over a real external QUIC peer;
// `rotation.persistent.certificate_binding_change` proves the
...
// under the *general* profile, whenever TLS files are configured -- the
// same store serves both TCP/H1 and QUIC/H3 there (#649 retired the
// #629 mixed OpenSSL-TCP/native-HTTP3 composition entirely; there is now
// exactly one credential owner per profile). So this composed proof drives
// real QUIC/H3 (the same `gtlsclient` subprocess harness `h3interop.quic.*`
// above uses) rather than TCP, matching the resumption cache/ticket surface
// this test cares about, but the credential store it exercises is the same
// one TCP/H1 shares.
//
// Bundles a credential swap with a broken ticket-key candidate in the same
// SIGHUP. `hotReloadConfig` runs `native_credentials.prepareReloadFromFiles`
// (staging the new credential, from B's bytes at the same configured path)
// before it reaches the ticket-key reload step -- `prepareReloadFromFiles`
// does not swap the live/active credential, only `commitPreparedReload`
// does, and that is reached only after every earlier reload step succeeds.
// The ticket-key step fails on the broken candidate and rejects the whole
// reload before `commitPreparedReload` is ever called, so the observable
// outcome (previous ticket state and served credential both unchanged)
// matches what this test always asserted, via ordinary reload atomicity
// rather than the retired mixed-owner special case.
//
...
test "proxy streaming mode reuses downstream connection after clean streamed response" {
    const allocator = std.testing.allocator;
    const payload_len = 128 * 1024;
    const first_payload = try allocator.alloc(u8, payload_len);
    defer allocator.free(first_payload);
    const second_payload = try allocator.alloc(u8, payload_len);
    defer allocator.free(second_payload);
    @memset(first_payload, 'a');
    @memset(second_payload, 'b');
    ...
    try sendKeepAliveGet(&stream, allocator, tardigrade.port, "/proxy/first.bin");
    var first = try readHttpResponse(allocator, stream);
    defer first.deinit();
    try std.testing.expectEqual(@as(u16, 200), first.status_code);
    try std.testing.expectEqualStrings("keep-alive", first.header("Connection").?);
    try std.testing.expectEqualStrings("chunked", first.header("Transfer-Encoding").?);
    try std.testing.expectEqualStrings(first_payload, first.body);

    try sendKeepAliveGet(&stream, allocator, tardigrade.port, "/proxy/second.bin");
    var second = try readHttpResponse(allocator, stream);
    defer second.deinit();
    try std.testing.expectEqual(@as(u16, 200), second.status_code);
    try std.testing.expectEqualStrings("keep-alive", second.header("Connection").?);
    try std.testing.expectEqualStrings("chunked", second.header("Transfer-Encoding").?);
    try std.testing.expectEqualStrings(second_payload, second.body);
    try std.testing.expectEqual(@as(u32, 2), upstream.requestCount());
}

test "proxy streaming mode reuses downstream connection after clean h2c upstream stream" {
    const allocator = std.testing.allocator;
    const payload_len = 8 * 1024;
    const payload = try allocator.alloc(u8, payload_len);
    defer allocator.free(payload);
    @memset(payload, 'h');

    var upstream = try H2cUpstreamServer.start(allocator, payload);
    defer upstream.stop();
    try upstream.run();
    ...
    try sendKeepAliveGet(&stream, allocator, tardigrade.port, "/proxy/h2c-first.bin");
    var first = try readHttpResponse(allocator, stream);
    defer first.deinit();
    try std.testing.expectEqual(@as(u16, 200), first.status_code);
    try std.testing.expectEqualStrings("keep-alive", first.header("Connection").?);
    try std.testing.expectEqualStrings("chunked", first.header("Transfer-Encoding").?);
    try std.testing.expectEqualStrings(payload, first.body);

    try sendKeepAliveGet(&stream, allocator, tardigrade.port, "/proxy/h2c-second.bin");
    var second = try readHttpResponse(allocator, stream);
    defer second.deinit();
    try std.testing.expectEqual(@as(u16, 200), second.status_code);
    try std.testing.expectEqualStrings("keep-alive", second.header("Connection").?);
    try std.testing.expectEqualStrings("chunked", second.header("Transfer-Encoding").?);
    try std.testing.expectEqualStrings(payload, second.body);
    try std.testing.expectEqual(@as(u32, 2), upstream.requestCount());
}

test "proxy streaming mode preserves downstream close policies" {
    const allocator = std.testing.allocator;
    ...
    var upstream = try UpstreamServer.start(allocator, &.{
        .{ .body = payload, .headers = &.{.{ .name = "Content-Type", .value = "application/octet-stream" }} },
        .{ .body = payload, .headers = &.{.{ .name = "Content-Type", .value = "application/octet-stream" }} },
        .{ .body = payload, .headers = &.{.{ .name = "Content-Type", .value = "application/octet-stream" }} },
    });
    defer upstream.stop();
    ...
    var tardigrade = try TardigradeProcess.start(allocator, .{
        ...
        .extra_env = &.{
            ...
            .{ .name = "TARDIGRADE_PROXY_STREAM_BUFFER_SIZE", .value = "4096" },
            .{ .name = "TARDIGRADE_MAX_BUFFERED_UPSTREAM_RESPONSE_BYTES", .value = "65536" },
            .{ .name = "TARDIGRADE_MAX_REQUESTS_PER_CONNECTION", .value = "2" },
            .{ .name = "TARDIGRADE_KEEP_ALIVE_TIMEOUT_MS", .value = "30000" },
        },
    });
    defer tardigrade.stop();
    ...
    {
        var close_response = try sendRequest(allocator, tardigrade.port, .{
            .method = "GET",
            .path = "/proxy/client-close.bin",
            .body = null,
            .headers = &.{},
        });
        defer close_response.deinit();
        try std.testing.expectEqual(@as(u16, 200), close_response.status_code);
        try std.testing.expectEqualStrings("close", close_response.header("Connection").?);
        try std.testing.expectEqualStrings("chunked", close_response.header("Transfer-Encoding").?);
        try std.testing.expectEqualStrings(payload, close_response.body);
    }
    ...
    try sendKeepAliveGet(&stream, allocator, tardigrade.port, "/proxy/max-first.bin");
    var first = try readHttpResponse(allocator, stream);
    defer first.deinit();
    try std.testing.expectEqual(@as(u16, 200), first.status_code);
    try std.testing.expectEqualStrings("keep-alive", first.header("Connection").?);

    try sendKeepAliveGet(&stream, allocator, tardigrade.port, "/proxy/max-second.bin");
    var second = try readHttpResponse(allocator, stream);
    defer second.deinit();
    try std.testing.expectEqual(@as(u16, 200), second.status_code);
    try std.testing.expectEqualStrings("close", second.header("Connection").?);

    var probe: [1]u8 = undefined;
    const n = try readSocketWithPoll(stream.handle, &probe, 5_000);
...
test "proxy streaming mode keeps bodiless downstream responses reusable when eligible" {
    const allocator = std.testing.allocator;
    ...
    const requests = [_]struct {
        ...
        .{ .method = "GET", .path = "/proxy/no-content", .status = 204 },
        .{ .method = "GET", .path = "/proxy/not-modified", .status = 304 },
    };
    ...
    for (requests) |request| {
        const raw_request = try std.fmt.allocPrint(
            allocator,
            "{s} {s} HTTP/1.1\r\nHost: {s}:{d}\r\nConnection: keep-alive\r\n\r\n",
            .{ request.method, request.path, test_host, tardigrade.port },
        );
        defer allocator.free(raw_request);
        try stream.writeAll(raw_request);

        const raw_head = try readHttpHeadersOnly(allocator, stream);
        defer allocator.free(raw_head);
        try std.testing.expectEqual(request.status, try parseStatusCode(raw_head));
        try std.testing.expectEqualStrings("keep-alive", headerValue(raw_head, "Connection").?);
        try std.testing.expect(headerValue(raw_head, "Transfer-Encoding") == null);
    }

    try sendKeepAliveGet(&stream, allocator, tardigrade.port, "/proxy/final");
    var final = try readHttpResponse(allocator, stream);
    defer final.deinit();
    try std.testing.expectEqual(@as(u16, 200), final.status_code);
    try std.testing.expectEqualStrings("keep-alive", final.header("Connection").?);
    try std.testing.expectEqualStrings("chunked", final.header("Transfer-Encoding").?);
    try std.testing.expectEqualStrings("final streamed body", final.body);
    try std.testing.expectEqual(@as(u32, 4), upstream.requestCount());
}
...
test "proxy streaming mode closes downstream after graceful upstream truncation on keepalive request" {
    const allocator = std.testing.allocator;

    var upstream = try UpstreamServer.start(allocator, &.{.{
        .body = "abcdef",
        .headers = &.{.{ .name = "Content-Type", .value = "text/plain" }},
        .declared_content_length = 6,
        .truncate_body_after = 3,
    }});
    defer upstream.stop();
    try upstream.run();
    ...
    defer stream.close();
    try setStreamTimeouts(&stream, 5_000);
    try stream.writeAll("GET /proxy/truncated HTTP/1.1\r\nHost: test\r\nConnection: keep-alive\r\n\r\n");
    ...
    try std.testing.expect(std.mem.find(u8, raw.items, "0\r\n\r\n") == null);

    stream.writeAll("GET /proxy/after-truncate HTTP/1.1\r\nHost: test\r\nConnection: close\r\n\r\n") catch {};
    var probe: [1]u8 = undefined;
    const n = readSocketWithPoll(stream.handle, &probe, 5_000) catch |err| switch (err) {
        error.ConnectionResetByPeer => @as(usize, 0),
        else => return err,
    };
    try std.testing.expectEqual(@as(usize, 0), n);
...
test "proxy streaming mode never serializes a second HTTP response after the downstream head is committed" {
    ...
    stream.writeAll("GET /proxy/after-reset HTTP/1.1\r\nHost: test\r\nConnection: close\r\n\r\n") catch {};
    var probe: [1]u8 = undefined;
    const n = readSocketWithPoll(stream.handle, &probe, 5_000) catch |err| switch (err) {
        error.ConnectionResetByPeer => @as(usize, 0),
        else => return err,
...
test "proxy streaming mode streams from an upstream requiring a client certificate" {
    const allocator = std.testing.allocator;
    // The native upstream HTTPS/TLS client (#634/#643) supports presenting
    // a configured client certificate (`UpstreamTlsOptions.client_cert_path`)
    // in every TLS profile — this is not OpenSSL-adapter-only behavior.
    try requireOpenssl(allocator);
...
// The incident configuration for the #791 tests below: trust enforced and
// locked down to explicit proxy CIDRs (loopback = the test client's socket
// peer, 10.0.0.0/8 = an inner proxy hop), never open trust.
const trusted_cidr_env = [_]EnvPair{
    .{ .name = "TARDIGRADE_TRUSTED_PROXY_CIDRS", .value = "127.0.0.0/8,10.0.0.0/8" },
    .{ .name = "TARDIGRADE_TRUST_REQUIRE_UPSTREAM_IDENTITY", .value = "true" },
};

test "spoofing the leftmost X-Forwarded-For entry cannot bypass per-IP rate limiting (#791)" {
    const allocator = std.testing.allocator;
    ...
    // Rotating the client-chosen leftmost entry must not buy a fresh bucket.
    for ([_][]const u8{ "203.0.113.2", "203.0.113.3" }) |spoof| {
        const xff = try std.fmt.allocPrint(allocator, "{s}, 198.51.100.20, 10.0.0.5", .{spoof});
        defer allocator.free(xff);
        var spoofed = try sendXffRateRequest(allocator, tardigrade.port, &.{
            .{ .name = "X-Forwarded-For", .value = xff },
        });
        defer spoofed.deinit();
        try std.testing.expectEqual(@as(u16, 429), spoofed.status_code);
    }

    // A second, client-supplied XFF field line must not shadow the proxy's.
    var split = try sendXffRateRequest(allocator, tardigrade.port, &.{
        .{ .name = "X-Forwarded-For", .value = "203.0.113.9, 198.51.100.99" },
        .{ .name = "X-Forwarded-For", .value = "198.51.100.20, 10.0.0.5" },
    });
    defer split.deinit();
    try std.testing.expectEqual(@as(u16, 429), split.status_code);
    try std.testing.expectEqual(@as(u32, 1), upstream.requestCount());
}

test "distinct legitimate client IPs get separate rate-limit buckets behind trusted proxies (#791)" {
    const allocator = std.testing.allocator;
    ...
    var a = try sendXffRateRequest(allocator, tardigrade.port, &.{
        .{ .name = "X-Forwarded-For", .value = "198.51.100.20, 10.0.0.5" },
    });
    defer a.deinit();
    try std.testing.expectEqual(@as(u16, 200), a.status_code);

    // Same spoofable leftmost prefix, different real client: own bucket.
    var b = try sendXffRateRequest(allocator, tardigrade.port, &.{
        .{ .name = "X-Forwarded-For", .value = "198.51.100.21, 10.0.0.5" },
    });
    defer b.deinit();
    try std.testing.expectEqual(@as(u16, 200), b.status_code);

    var a_again = try sendXffRateRequest(allocator, tardigrade.port, &.{
        .{ .name = "X-Forwarded-For", .value = "198.51.100.20, 10.0.0.5" },
    });
    defer a_again.deinit();
    try std.testing.expectEqual(@as(u16, 429), a_again.status_code);
    try std.testing.expectEqual(@as(u32, 2), upstream.requestCount());
}

test "trusted proxy real-IP header wins over contradictory X-Forwarded-For (#791)" {
    const allocator = std.testing.allocator;
    ...
    const env = trusted_cidr_env ++ [_]EnvPair{
    ...
    // Same real-IP header, different XFF: same bucket.
...
test "untrusted peer cannot set the client IP or forward a forged real-IP header to the origin (#791)" {
    ...
    var tardigrade = try TardigradeProcess.start(allocator, .{
        ...
        .ready_path = "/healthz",
        .extra_env = &.{
            .{ .name = "TARDIGRADE_TRUSTED_PROXY_CIDRS", .value = "10.0.0.0/8" },
            .{ .name = "TARDIGRADE_TRUST_REQUIRE_UPSTREAM_IDENTITY", .value = "true" },
            .{ .name = "TARDIGRADE_REAL_IP_HEADER", .value = "CF-Connecting-IP" },
        },
    });
...
test "HSTS header is emitted on HTTPS responses when enabled (#175)" {
    ...
    var response = try sendPureZigTlsHttp1Request(allocator, tardigrade.port, "/healthz");
    defer response.deinit();
    try std.testing.expectEqual(@as(u16, 200), response.status_code);
...
// #649: retired "TLS 1.1 client is rejected when the minimum version is
// 1.2 (#175)" -- it relied on `TARDIGRADE_TLS_MIN_VERSION` defaulting to
// "1.2" against a real OpenSSL terminator honoring that floor. The native
...
const PersistentRawPeerResponder = struct {
    origin: *OpensslRawPeer,
    allocator: std.mem.Allocator,
    response_bytes: []const u8,
    expected_requests: usize,
    observed_requests: usize = 0,
    err: ?anyerror = null,

    fn run(self: *PersistentRawPeerResponder) void {
        while (self.observed_requests < self.expected_requests) {
            self.origin.waitForRequestHead(self.allocator, 10_000) catch |err| {
                self.err = err;
                return;
            };
...
test "native upstream https: two proxied requests reuse the pooled TLS connection" {
    ...
    const body = "native-upstream-reuse-body";
    const response_bytes = try std.fmt.allocPrint(
        allocator,
        "HTTP/1.1 200 OK\r\nContent-Length: {d}\r\nConnection: keep-alive\r\n\r\n{s}",
        .{ body.len, body },
    ...
    var responder = PersistentRawPeerResponder{
        .origin = &origin,
        .allocator = allocator,
        .response_bytes = response_bytes,
        .expected_requests = 2,
    ...
    if (compat.getEnvVarOwned(allocator, "TARDIGRADE_NATIVE_TLS_REUSE_SOAK_DIAGNOSTICS")) |enabled| {
        defer allocator.free(enabled);
        if (std.mem.eql(u8, enabled, "1")) {
            std.debug.print(
                "native upstream TLS pool reuse diagnostics: new={d} reused={d} stale={d} plaintext_unexpected={d} tls_application_plaintext={d} tls_peer_closed={d} tls_drive_error={d} tls_drain_budget={d} quarantine_incomplete={d} release_lifetime={d} release_capacity={d} release_not_reusable={d}\n",
...
// reload-rejection tests above. On the general profile the credential
